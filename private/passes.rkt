...
(define-pass identify-module-variables : Lsyntax (e) -> Lmodulevars ()
  (SubmoduleForm : submodule-form (e) -> submodule-form ()
                 [(module ,id ,module-path
                    ...
                    (,[submodule-form] ...)
                    (,[submodule-form*] ...))
                  `(module ,id ,module-path (prefix (,(apply set-union '() bindings) ...))
                     (,raw-provide-spec ...)
                     (,raw-require-spec ...)
                     (,module-level-form ...)
                     (,syntax-level-form ...)
  ...
  (SyntaxBody : syntax-body (e) -> syntax-body ()
              ...
              [(begin-for-syntax ,[module-level-form bindings] ...)
               `(begin-for-syntax (prefix (,(apply set-union '() bindings) ...))
                                  ,module-level-form ...)])
  (ModuleLevelForm : module-level-form (e) -> module-level-form ('()))
  (GeneralTopLevelForm : general-top-level-form (e) -> general-top-level-form ('())
  ...
  (TopLevelForm : top-level-form (e) -> top-level-form ('())
                [(begin* ,[top-level-form assigned] ...)
                 (values `(begin* ,top-level-form ...)
                         (apply set-union '() assigned))]
                [(begin-for-syntax* ,[prefix-form] ,[top-level-form assigned] ...)
                 (values `(begin-for-syntax* ,prefix-form ,top-level-form ...)
                         (apply set-union '() assigned))]
                [(#%expression ,[expr assigned])
                 (values `(#%expression ,expr)
                         assigned)]
                [(define-syntaxes* (,v ...) ,[prefix-form] ,[expr assigned])
                 (values `(define-syntaxes* (,v ...) ,prefix-form ,expr)
  ...
  (TopLevelForm : top-level-form (e [env '()]) -> top-level-form ('() '())
                [(begin* ,[top-level-form free-local free-global] ...)
                 (values `(begin* ,top-level-form ...)
                         (apply set-union '() free-local)
                         (apply set-union '() free-global))]
                [(begin-for-syntax* ,[prefix-form] ,[top-level-form free-local free-global])
                 (values `(begin-for-syntax* ,prefix-form ,top-level-form ...)
                         (apply set-union '() free-local)
                         (apply set-union '() free-global))]
                [(#%expression ,[expr free-local free-global])
                 (values `(#%expression ,expr) free-local free-global)]
                [(define-syntaxes* (,v ...) ,[prefix-form] ,[expr free-local free-global])
                 (values `(define-syntaxes* (,v ...) ,prefix-form ,expr)
                         free-local
                         (set-union free-global v))])
...
(define-pass debruijn-indices : Lreintroducesyntax (e) -> Ldebruijn ()
  (definitions
    ...
    (define (lookup-env env id)
      (dict-ref env id))
    (define (make-global-env prefix)
      (define ids (nanopass-case (Ldebruijn prefix-form) prefix
                                 [(prefix (,binding ...) (,stx ...)) binding]))
      (for/fold ([env (hash)])
...
(define-pass find-let-depth : Ldebruijn (e) -> Lfindletdepth ()
  (PrefixForm : prefix-form (e depths) -> prefix-form ()
              [(prefix (,binding ...) (,stx ...))
               `(prefix (,binding ...) (,stx ...) ,(apply max 1 depths))])
  ...
  (TopLevelForm : top-level-form (e) -> top-level-form (0)
                ...
                [(begin* ,[top-level-form depth] ...)
                 (values `(begin* ,top-level-form ...)
                         (apply max 0 depth))]
                [(begin-for-syntax* ,prefix-form ,[top-level-form depth] ...)
                 (values `(begin-for-syntax* ,(PrefixForm prefix-form (apply max 1 depth))
                                             ,top-level-form ...)
                         0)]
                [(define-syntaxes* (,v ...) ,prefix-form ,[expr depth])
                 (values `(define-syntaxes* (,v ...) ,(PrefixForm prefix-form (list depth)) ,expr)
                         0)])
